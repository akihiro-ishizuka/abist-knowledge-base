...
| **フルコーパス規模で新たに出現、hybrid限定、keyword/short_term起因(vector不関与)** | **6件** | **12%** | **原因特定済み(下記)。q19/q22と同じrowidタイブレーク非決定性がtop-60候補プールの尾部で発生** |

**この6件は追加調査で原因特定済み(`.superpowers/sdd/M4-index-search/citation-residual-diagnosis.md`
参照)。** Stage A(索引固定・`task-4-5-report.md`)が既に22クエリ全問・両方式で
baselineとビット同一(出典行一致356/356=100.0%)を実測しており、検索エンジンの
ロジック(クエリ語抽出・BM25式・RRF・ブースト・重複排除)自体には欠陥が無いことは
確定していた。今回、索引だけを再構築した状態(埋め込みは不要、`keyword_search`/
`short_term_search`のみ)でこの6クエリの`candidate_limit=60`候補プールを
baselineの`bm25_raw.raw`(60件)と突き合わせたところ、q10で同一文書の別チャンクが
プール境界(ニアタイのスコア帯)で入れ替わっていることを実測した——これは
q19/q22で既に特定済みの「索引再構築でrowid割当てが変わり、同点/僅差内の並び順が
変わる」非決定性と**同一メカニズム**であり、違いはtop-10(bm25_rawの最終出力)
ではなくtop-60(RRFへの候補プールの尾部)で起きている点だけである。top-60の
尾部での入れ替わりはbm25_raw自身の上位10件には影響しない(実際96.9%で高いまま)が、
RRFがその60件全体を使うため、hybridの折り畳み結果にだけ波及する。6クエリ中1件
(部分埋め込みで再現)で「baselineと同一スコアの共有エントリが1つずつ後ろへ
ずれ、新規参入エントリがrank10を押し出す」パターンを数値レベルで確認しており、
RRFの計算式自体は同一であること(共有エントリのスコアがビット単位で一致)も
裏付けられた。**分類: 索引再構築の正当な帰結であり、我々の検索ポートの
アルゴリズム的な欠陥ではない。修正しない(閾値も緩めない)。**

**結論**: 設計が最初に挙げ最も重く扱う基準であるRecall@5は全チャンク規模で
ビット同一のまま合格している一方、出典行一致率は不合格。50件の不一致は
すべて原因特定済み——37件は埋め込み差異(既知)、5件+6件は同一のrowidタイ
ブレーク非決定性(bm25_rawのtop-10で顕在化するか、RRF候補プールのtop-60尾部で
顕在化するかの違いのみ)。§15の検索品質受入条件(`design/plans/M6-M10-remaining.md`
