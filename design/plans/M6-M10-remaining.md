...
埋め込み差異、5件(10%)は既知のBM25タイブレーク非決定性で説明できるが、
**6件(12%)はフルコーパス規模で新たに出現したhybrid限定・keyword/short_term
起因の未解明の残差**であり、埋め込み差異でもタイブレークでもない。RRFの
keyword候補プールがフルコーパス規模で広がることが原因ではという仮説はあるが
未検証。**このチェックリスト項目をサインオフする際は、Recall@5合格と出典行
一致率不合格を混同せず、6件の残差が未解明のままであることを判断材料として
明示すること。閾値を緩めて帳尻を合わせてはならない。**
