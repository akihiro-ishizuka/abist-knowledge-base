...
**出典は受け手が開ける URL であること。** esa 由来の記事は frontmatter の `url`（`https://abist.esa.io/posts/NNNN`）を書き、`docs/esa/...` のようなリポジトリのパスは書かない。メンバーはこのリポジトリを持っておらず開けない。出典の目的は受け手が自分で確かめられることなので、確かめられない文字列を添えると、根拠があるように見えて実際には無い状態になり **出典が無いより悪い**。
...
チャットID で絞れないため、**送信者で絞った検索を active 6名それぞれに1本ずつ**投げ、結果を統合する。

```
query = "from:<active メンバーのメールアドレス>"   # 6本
afterDateTime は付けない
```

#### 5.1.0 日時フィルタを付けてはならない（2026-08-21 確定）

`chat_message_search` には2つの内部経路があり、**日時フィルタは絞り込みではなく経路の切り替えスイッチ**である。

| | 日時フィルタ無し（A経路） | 日時フィルタ有り（B経路） |
|---|---|---|
| 実体 | Graph 全文検索インデックス | 50チャット×50件の総なめ（最大2500件） |
| `query` | KQL（`from:` が効く） | 単なるリテラル部分一致 |
| `from.email` | 入る | **`null`** |
| `429` | 観測されず | **頻発**（稼働初日に5回中3回） |

**B経路は `429` でもエラーを返さない。**「結果は部分的」という注記を付けて `200` で返るため、**新着0件と区別できない**。完全性を時刻カーソルに預けた設計はここで静かに取りこぼす。

本設計は新着判定を `message_id` だけで行い、時刻は検索の下限を決める目安にしか使っていない（§5.1）。したがって A経路の並び順は問題にならず、**日時フィルタは最初から不要だった**。

`from:` で絞ることで、25件の枠を対象メンバーの発言で埋められる。他の忙しいチャットに枠を奪われて連絡チャットが埋もれる事故（2026-08-21 に実測）を防げる。

**代償**: context-only メンバーの発言は拾えない。文脈参照はできなくなるが、取りこぼして害があるのは active 6名の発言だけなので妥当な交換とする。

初期案（高頻度のかなを probe として投げる方式）は、B経路を前提にした設計であり破棄した。稼働初日に `429` で半数のティックが死に、probe を3語から1語へ減らしても解消しなかった。

#### 5.1.1 検索は逐次に投げる

**6本の検索を並行に投げてはならない。** 1本の応答を受け取ってから次を投げる。

2026-08-21 の実測で、同じ経路・同じクエリでも投げ方だけで結果が分かれた。

| 投げ方 | 結果 |
|---|---|
| 1本ずつ逐次 | 6本すべて成功、`429` ゼロ |
| 3本を同時 | 3本すべて `429` |

`Retry-After` は一貫して62秒であり、制限は「短い窓での同時リクエスト数」に効いているとみられる。経路の問題ではなく呼び出し方の問題なので、A経路への切り替え判断は有効なまま。

`limit` は 10 とする。結果は新しい順に返るため、前ティックからの新着を拾うのに 25 件は不要で、6名分では取得量がコンテキストを圧迫するだけになる。
