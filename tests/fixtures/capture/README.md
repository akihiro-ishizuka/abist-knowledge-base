...
各カテゴリの契約(ビット互換/advisory)・旧テスト28ファイルへの転記カバレッジ・
既知のギャップと除外・旧システムのデータ層に関する訂正事実は
[`../PROVENANCE.md`](../PROVENANCE.md) にまとめてある(このディレクトリの説明は
再採取の手順、あちらは由来と契約の恒久記録)。
...
2. **`{readonly: true, fileMustExist: true}` オプション方式 — 使ってはならない。**
   かつて「このオプションなら mtime を変化させずに開ける」と記載していたが、
   **実測で誤りであることが確認された**(2026-08-04)。実ファイルに対して
   このオプションで開いたところ、`data/kb-index.sqlite-shm` の mtime が変化した
   (サイズは 32768 バイトのまま)。`-shm` は WAL のインデックスであり、本体と
   WAL から決定的に再生成できる一時ファイルではあるが、`assertReadOnly()` は
   これを変更として正しく検出する。**方式1(サンドボックスコピー)のみを使うこと。**
