...
| MCP replay(`tests/mcp/replay.py`)は `tool_result_json` について**構造のみを検証し、値は検証しない** | `_structural_mismatch` はキー集合・大まかな型のみを再帰比較し、値そのものは比較しない。これは手落ちではなく、比較対象がないことによる制約: このリポジトリのテストは生産環境の `docs/`・`data/*.sqlite`(`search_kb`/`get_document`/`get_chunk`/`index_status` 等の fixture が実測した対象)にアクセスできず(`tests/search/test_services.py` と同じ制約)、契約テストは合成した小さな work/reference コーパスに対して実行する。したがって fixture が記録したパス・件数・スコア・`tokenizer` 判定・`vectorAvailable` 等の**実測値そのものと1対1で一致することは原理的にありえない**。値を強制一致させると、真の契約違反ではなく単に「テストコーパスが違う」ことを理由に恒常的に赤くなる | 「MCPの契約が replay で完全検証されている」と読むのは誤り。replay が保証するのは **JSON-RPC 応答の形(キー集合・必須性・大まかな型)が変わっていないこと** だけであり、**返す値が正しいこと**は保証しない。値の正しさは別の単体テスト(`tests/search/test_services.py` 等)が担う。例外として `replay.py` の `_STATIC_VALUE_FIELDS` に列挙した少数のフィールドだけは値まで比較する — 具体的には現時点で `kb-search/get_document/path_traversal_attempt` の `error`(`"docs/ の外は参照できません"`、`kb_search.py` のコード上のリテラル文字列で、引数のパスも索引の中身も一切含まない)の1件のみ。調査の結果、他の error 系フィールド(`nonexistent_path`/`nonexistent_chunk_id`/`invalid_url_format`/`run_batch.unknown_batch_name` 等)はいずれも引数値・サンドボックスの絶対パス・DB上のバッチ名一覧など環境/データ依存の内容をメッセージに埋め込んでおり、値比較の対象にできない(埋め込み先を広げる際は、その値が本当にデータ非依存であることを個別に確認すること) |
...
## 6. 旧システムは「変更しない」が「静止してもいない」

**この節が記録するのは、これまでの「旧リポジトリを変更しない」という規律とは別の、独立した事実である。** §5は「我々の作業が旧リポジトリを変更しないこと」の規律であり、これはずっと守られている(採取スクリプトは全て読み取り専用、`assertReadOnly()` のフィンガープリント検証も一貫して通過)。しかし2026-08-03、マイルストーン作業と並行して、旧システム自身のesaツール(利用者が日常運用している別プロセス)が `docs/チーム内定例/設計効率化/` 配下に18文書を書き込み、`data/sync-state.sqlite` を2,654,208バイトから3,239,936バイトへ成長させた。追跡対象ファイルの `git status` は75行のベースラインのまま不変で、変化した対象はすべて `.gitignore` 対象の `docs/`・`data/` 配下である。

**「旧リポジトリを変更しない」は「我々の作業が触れていない」ことを意味するのであって、「旧システムが静止している」ことを意味しない。** 本プロジェクトの全ての検証記述はこの前者の意味で書かれている。後者の意味に読み違えないよう、ここに明記する。旧システムは誰かが日常的に使っている実運用システムであり、こちらの作業とは無関係に動き続ける。

この事実には3つの具体的な帰結がある:

- **fixture は「ある時点のスナップショット」であり「旧システムの鏡」ではない。** `tests/fixtures/**` の値は2026-08-03に採取された時点の記録として有効であり続けるが、同じ採取スクリプトを後日再実行すれば異なる値が得られて当然であり、それは旧システムが動いた結果であって採取スクリプトの不具合ではない。今回変化した18文書は `tests/fixtures/real-docs/samples.json` の40件の実文書サンプルには含まれていないことを確認済みであり、現時点で採取済みのfixtureに陳腐化した値は無い。
- **移行(M8/M9)は「その瞬間の状態」に対して実行しなければならない。** `abist-kb migrate inspect` は、実際の移行を実行する直前に必ず再実行し、事前に測っておいた数値を信用してはならない。これは M8 レポートが別の理由(スキャン対象に紛れ込んだ同梱仮想環境)からすでに導いていた同じ結論に対する、今回判明した独立の第2の理由である。二重の根拠がある以上、この運用ルールの重要度は高い。
- **並行稼働期間中の監視(`design/plans/M9-cutover-runbook.md` §4)は、この意味での「差分」を旧システムの誤動作と誤認しないこと。** `git status --porcelain` 行数や `data/*.sqlite` のサイズ変化は、Python 版の書き込みだけでなく、旧システム自身の日常利用によっても発生しうる。
